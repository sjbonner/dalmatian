...
The first line in each of the following snippets of code (commented out) shows how the functions were applied to the raw output and the following lines display the results. The sample MCMC output is available in the source code repository on GitHub and can be downloaded from https://github.com/sjbonner/dalmatian/blob/issue54/data-mcmc/nbmcmc.RData .

## Convergence Diagnostics
```{r}
## Compute convergence diagnostics
## nbresults$convergence <- convergence(nbmcmc)

## Gelman-Rubin diagnostics
nbresults$convergence$gelman

## Raftery diagnostics
nbresults$convergence$raftery

## Effective sample size
nbresults$convergence$effectiveSize
```

## Traceplots
```{r,fig.width=6,fig.align="center"}
## Generate traceplots
## nbresults$traceplots <- traceplots(nbmcmc,show=FALSE)

## Fixed effects for mean
nbresults$traceplots$meanFixed

## Fixed effects for dispersion
nbresults$traceplots$dispersionFixed
```

## Numerical Summaries
```{r}
## Compute numerical summaries
## nbresults$summary <- summary(nbmcmc)
...
## nbresults$caterpillar <- caterpillar(nbmcmc,show = FALSE)

## Fixed effects for mean
nbresults$caterpillar$meanFixed

## Fixed effects for dispersion
nbresults$caterpillar$dispersionFixed
```

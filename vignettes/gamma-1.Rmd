...
The first line in each of the following snippets of code (commented out) shows how the functions were applied to the raw output and the following lines display the results. The sample MCMC output is available in the source code repository on GitHub and can be downloaded from https://github.com/sjbonner/dalmatian/blob/issue54/data-mcmc/gmcmc.RData .

## Convergence Diagnostics
```{r}
## Compute convergence diagnostics
## gresults$convergence <- convergence(gmcmc)

## Gelman-Rubin diagnostics
gresults$convergence$gelman

## Raftery diagnostics
gresults$convergence$raftery

## Effective sample size
gresults$convergence$effectiveSize
```

## Traceplots
```{r,fig.width=6,fig.align="center"}
## Generate traceplots
## gresults$traceplots <- traceplots(gmcmc,show=FALSE)

## Fixed effects for mean
gresults$traceplots$meanFixed

## Fixed effects for dispersion
gresults$traceplots$dispersionFixed
```

## Numerical Summaries
```{r}
## Compute numerical summaries
## gresults$summary <- summary(gmcmc)
...
## gresults$caterpillar <- caterpillar(gmcmc,show = FALSE)

## Fixed effects for mean
gresults$caterpillar$meanFixed

## Fixed effects for dispersion
gresults$caterpillar$dispersionFixed
```

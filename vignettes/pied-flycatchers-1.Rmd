...
```{r, echo = FALSE}
load(system.file("pfresults.RData",package="dalmatian"))
```
...
The first line in each snippets of code illustrating these functions (commented out) shows how the functions were applied to the raw output. The following lines display the results. The sample MCMC output is available in the source code repository on GitHub and can be downloaded from https://github.com/sjbonner/dalmatian/blob/issue54/data-mcmc/pfmcmc.RData .
...
## pfresults$convergence <- convergence(pfmcmc)

## Gelman-Rubin diagnostics
pfresults$convergence$gelman

## Raftery diagnostics
pfresults$convergence$raftery

## Effective sample size
pfresults$convergence$effectiveSize
```
...
## Generate traceplots
## pfresults$traceplots <- traceplots(pfmcmc,show=FALSE,nthin=20)

## Fixed effects for mean
pfresults$traceplots$meanFixed

## Fixed effects for dispersion
pfresults$traceplots$dispersionFixed
...
## pfresults$summary <- summary(pfmcmc)
...
## pfresults$caterpillar <- caterpillar(pfmcmc,show = FALSE)

## Fixed effects for mean
pfresults$caterpillar$meanFixed

## Fixed effects for dispersion
pfresults$caterpillar$dispersionFixed
```
...
## Results
Once again, we can use the wrapper functions within the package to examine the convergence and compute summaries of the posterior distribution. 

As before, the first line in each snippets of code illustrating these functions (commented out) shows how the functions were applied to the raw output. The following lines display the results. The sample MCMC output is available in the source code repository on GitHub and can be downloaded from https://github.com/sjbonner/dalmatian/blob/issue54/data-mcmc/pfmcmc2.RData .

1. Convergence diagnostics
```{r}
## Compute convergence diagnostics
## pfresults2$convergence <- convergence(pfmcmc2)

## Gelman-Rubin diagnostics
pfresults2$convergence$gelman

## Raftery diagnostics
pfresults2$convergence$raftery

## Effective sample size
pfresults2$convergence$effectiveSize
```

2. Traceplots
```{r,fig.width=6,fig.align="center"}
## Generate traceplots
## pfresults$traceplots2 <- traceplots(pfmcmc2,show = FALSE,nthin=20)

## Fixed effects for mean
pfresults2$traceplots2$meanFixed

## Fixed effects for dispersion
pfresults2$traceplots2$dispersionFixed

## Random effects variances for mean
pfresults2$traceplots2$meanRandom

## Random effects variances for dispersion
pfresults2$traceplots2$dispersionRandom
```

3. Numerical summaries
```{r}
## Compute numerical summaries
## pfresults2$summary <- summary(pfmcmc2)
...
```{r,fig.width=6,fig.align="center"}
## Generate caterpillar
## pfresults2$caterpillar <- caterpillar(pfmcmc2,show = FALSE)

## Fixed effects for mean
pfresults2$caterpillar$meanFixed

## Fixed effects for dispersion
pfresults2$caterpillar$dispersionFixed
```
...
## pfresults$fitted <- predict(pfmcmc)

head(pfresults$fitted$mean)
head(pfresults$fitted$dispersion)

# Compute predictions for model 2
## pfresults2$fitted <- predict(pfmcmc2)

head(pfresults2$fitted$mean)
head(pfresults2$fitted$dispersion)

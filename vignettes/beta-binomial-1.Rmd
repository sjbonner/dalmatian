...
$$
g(\mu_i)=\eta_i=\bm x_{\mu,i}^\top \bm \alpha + \bm z_{\mu,i}^\top \bm \epsilon
$$
and 
$$
h(\phi_i)=\lambda_i= \bm x_{\sigma,i}^\top \bm \psi + \bm z_{\sigma,i}^\top \bm \xi
$$
with $\bm \alpha$ and $\bm \epsilon$ representing vectors of fixed and random effects for the mean and $\bm \psi$ and $\bm \xi$ fixed and random effects for the dispersion. Note that both $\mu_i$ and $\phi_i$ must be between 0 and 1, and so it makes sense to assume that both $g(\cdot)$ and $h(\cdot)$ follow logistic links, or some other link function that maintains this constraint. 
